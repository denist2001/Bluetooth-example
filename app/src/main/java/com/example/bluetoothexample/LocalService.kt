...
open class LocalService<S : LocalService<S>> : Service() {
    ...
    abstract class Binding<S : LocalService<S>> protected constructor(val context: Context) {
        ...
        fun unbind() {
            if (boundOK) {
                ...
                try {
                    ...
                    Log.d("", "LocalService unbound from $context")
                } catch (e: Exception) {
                    Log.e("", "LocalService problem unbinding service.")
        ...
        fun bind(): Boolean {
            ...
            if (boundOK) {
                Log.d("", "LocalService bound to $context")
            } else {
                Log.d("","LocalService could not bind to $context")
            }
            return boundOK
        }
